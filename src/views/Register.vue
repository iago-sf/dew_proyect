...
<div class="bg-black h-screen py-[7%]">
    <div class="i-login/register-box">
        ...
        <p tabindex="0" class="focus:outline-none text-sm mt-4 font-medium leading-none text-gray-200">
            ...
            <router-link to="/login" class="hover:text-gray-200 focus:text-gray-200 focus:outline-none focus:underline hover:underline text-sm font-medium leading-none text-red-600 cursor-pointer">
                Sign in here
            </router-link>
        </p>
        <button aria-label="Continue with google" role="button" class="focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-200 py-3.5 px-4 border rounded-lg border-gray-200 flex items-center w-full mt-10">
        ...
        <div>
            <label id="username" class="text-sm font-medium leading-none text-red-600">
                Username
            </label>
            <input aria-labelledby="username" type="username" class="i-login/register-input" v-model.trim="username"/>
        </div>
        <div class="mt-6 w-full">
            <label id="email" class="text-sm font-medium leading-none text-red-600">
                Email
            </label>
            <input aria-labelledby="email" type="email" class="i-login/register-input" v-model.trim="email"/>
        </div>
        <div class="mt-6 w-full">
            ...
            <div class="relative flex items-center justify-center">
                <input id="pass" type="password" class="i-login/register-input" v-model.trim="password"/>
        ...
        <div v-if="error == 'duplicated'" class="mt-6 w-full border-2 border-red-600 rounded p-2 bg-red-600/20">
            <label for="pass" class="text-sm font-medium leading-none text-red-600">
...
async function register() {
    /* const { data } = await axios.post('http://localhost:3001/user/manage', this.data);
    ...
    // Firebase auth
    const auth = getAuth();
    const user = await createUserWithEmailAndPassword(auth, this.email.value, this.password.value)
        .then(() => {
